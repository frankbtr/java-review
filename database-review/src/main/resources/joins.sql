...
select first_name, last_name, substring(first_name,1,1)||'.'||substring(last_name,1,1) from employees;
...
select count(*), department, max(salary), min(salary), sum(salary) from employees group by department;

-- list the departments where they have more than 40 employees
select count(*), department from employees group by department having count(*) >40 order by department;

select count(*), department from employees where department <> 'Garden' group by department having count(*) > 40 order by department;
...
select count(*), substring(email,position('@' in email) +1) as email_domain from employees where email is not null group by email_domain
...
select first_name, email, division from employees e join departments d on e.department = d.department where email is not null;

-- list firstname, email, division, country fol all employees
select first_name, email, division, country from employees e join departments d on e.department = d.department join regions r on e.region_id = r.region_id;

select distinct department from employees;

-- join == inner join
select distinct e.department, d.department from employees e join departments d on e.department = d.department;

select distinct e.department, d.department from employees e left outer join departments d on e.department = d.department where d.department is null;

select distinct e.department, d.department from employees e right outer join departments d on e.department = d.department;

select distinct e.department, d.department from employees e full outer join departments d on e.department = d.department;
...
-- can we get same result with other way?
select * from employees where department in ('Computers', 'Device Repair', 'Phones & Tablets');
